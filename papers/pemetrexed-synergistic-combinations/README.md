...
- `preclinical_synergy` — explicit demonstration of synergy/sensitization/potentiation (combination index / Chou-Talalay / isobologram, or resistance reversal). **286 papers.**
- `clinical_landmark` — practice-defining or registrational clinical combination trials (Phase III / randomized / KEYNOTE / registrational). **79 papers.**
...
- `index.tsv` — curated metadata for all **365** papers (category, evidence, authors, title, venue, year, PMID, DOI, PMCID, URL, topic tags, download status).
- `fulltext/` — Europe PMC open-access full-text XML (**148** papers) retrieved via the EBI REST `fullTextXML` endpoint.
...
Eleven PubMed E-utils queries (pemetrexed × synergy; × immunotherapy; × antiangiogenic; × EGFR-TKI; × ALK/MET; × DDR; × platinum/mechanism; × signaling/apoptosis; × folate/TS; × radiation; × repurposed) were combined and deduplicated (**2,560** candidates). Records were scored by regex classification into partner classes; papers were retained if they showed **explicit synergy/sensitization evidence** or were **landmark clinical combination trials**, then off-topic records were pruned, yielding **365** papers (2001–2026; 174 from 2018 onward). KEYNOTE-189 was added explicitly as the foundational IO + pemetrexed trial.

## Full-text status

- `fulltext_xml`: **148** papers (Europe PMC open-access XML).
- `metadata_only`: **217** papers (catalogued from title + abstract + DOI/PMID/PMCID).
