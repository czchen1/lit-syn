"""Render the collection (report + domain notes + reference appendix) to a single PDF."""
...
OUT = os.path.join(HERE, "gd2_cart_toxicity_review.pdf")
...
def references():
    ...
    parts = ["<h1>Appendix: full reference list</h1>",
             f"<p>All {len(rows)} curated records, grouped by primary toxicity domain and ordered by "
...
def main():
    title = ("<h1 class='title'>GD2 CAR-T toxicity</h1>"
             "<p class='subtitle'>Clinical and preclinical safety, intracerebroventricular delivery, "
             "and transient organ-function change relevant to concomitant therapy</p>"
             "<p class='subtitle'>Literature synthesis of 852 curated records (2003&ndash;2026)</p>")
