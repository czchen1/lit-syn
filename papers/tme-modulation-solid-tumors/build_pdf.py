...
OUT = os.path.join(HERE, "tme_modulation_solid_tumors_review.pdf")
...
def main():
    ...
    title = ("<h1 class='title'>Modulating the tumour microenvironment in solid and "
             "immunotherapy-resistant tumours</h1>"
             "<p class='subtitle'>A literature synthesis across 19 mechanistic and "
             "interventional axes</p>"
             f"<p class='subtitle'>{len(rows)} curated records, {min(years)}&ndash;{max(years)} "
             f"&middot; {ev['clinical_trial']} clinical trials &middot; "
             f"{ev['translational_human']} human translational &middot; "
             f"{ev['preclinical'] + ev['mechanistic']} preclinical/mechanistic &middot; "
             f"{ev['review'] + ev['systematic_review']} reviews &middot; "
             f"{resist} resistance-focused</p>")
    body = [title]
